...
export default function CafeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const historyItems = useMemo(() => {
    ...
    const journalItems = (cafe.journal || []).map((j: any, idx: number) => ({
  ...
  const handleAddNote = () => {
    ...
    let updatedJournal = [...(cafe.journal || [])];
  ...
  const handleDeleteNote = (idx: number) => {
    if (!docRef || !cafe.journal) return;
    if (window.confirm("Confirm deletion of this note?")) {
      const updatedJournal = cafe.journal.filter((_: any, i: number) => i !== idx);
      updateDocumentNonBlocking(docRef, { journal: updatedJournal });
