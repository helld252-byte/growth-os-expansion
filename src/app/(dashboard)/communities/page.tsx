...
type CommunityFilter = 'active' | 'outreach' | 'prospecting' | 'all';
...
export default function CommunitiesPage() {
  ...
  const verticalTypes = ["Forum", "Blog", "Review Site"];
  ...
  const filtered = (partners || []).filter(p => {
    ...
    if (activeFilter === 'outreach') return p.status === 'Outreach';
    if (activeFilter === 'prospecting') return p.status === 'Prospecting';
    return true;
  });

  const counts = {
    'active': (partners || []).filter(p => verticalTypes.includes(p.type) && p.status === 'Active').length,
    'outreach': (partners || []).filter(p => verticalTypes.includes(p.type) && p.status === 'Outreach').length,
    'prospecting': (partners || []).filter(p => verticalTypes.includes(p.type) && p.status === 'Prospecting').length,
      ...
      <aside className="w-56 shrink-0 flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-8">
          <div className="flex flex-col gap-3">
            <h3 className="text-[9px] font-semibold uppercase tracking-[0.25em] text-tier-4 px-3 mb-1">Community Filters</h3>
      ...
      <main className="flex-1 flex flex-col gap-10">
        <header className="flex items-center justify-between">
          <div className="flex flex-col gap-2.5">
            <h1 className="text-3xl font-semibold tracking-tight text-tier-1">Community & Blogs</h1>
        ...
        <div className="flex flex-col gap-4">
          {isLoading ? (
            ...
            <div className="h-72 border border-dashed border-white/5 rounded-2xl flex flex-col items-center justify-center gap-4 opacity-30">
              <Users2 className="size-12 text-tier-3" />
              <span className="text-[11px] font-medium uppercase tracking-[0.25em] text-tier-3">No community units found</span>
            </div>
          ) : (
            filtered.map((p) => (
...
function CommunityListItem({ community }: { community: any }) {
    ...
    <div className="premium-panel p-6 rounded-2xl flex items-center justify-between group hover:border-primary/30 cursor-pointer transition-all active:scale-[0.995]">
      <div className="flex items-center gap-7">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-6">
            <Badge variant="outline" className={cn("border px-3 py-0.5 h-auto text-[10px] font-medium uppercase tracking-[0.12em] rounded-lg flex items-center gap-2 transition-all", getStatusStyles(community.status))}>
              <span className={cn("size-1.5 rounded-full", community.status === 'Active' ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-current")} />
              {community.status}
            </Badge>
            <div className="flex items-center gap-2">
              <Star className="size-4 text-amber-500" />
              <span className="text-[12px] font-medium tracking-tight text-tier-1 opacity-90">Trust: {community.impactScore}/10</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-12">
        <div className="flex flex-col text-right">
          <span className="text-[9px] font-semibold uppercase tracking-[0.25em] text-tier-4 mb-1.5">Last Interaction</span>
          <span className="text-[14px] font-medium tracking-tight text-tier-2">
            {community.lastContact || 'N/A'}
          </span>
        </div>
