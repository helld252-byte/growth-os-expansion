...
export default function CommunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const historyItems = useMemo(() => {
    ...
    const journalItems = (community.journal || []).map((j: any, idx: number) => ({
  ...
  const handleOpenEditNote = (idx: number) => {
    const note = community.journal[idx];
  ...
  const handleAddNote = () => {
    ...
    let updatedJournal = [...(community.journal || [])];
  ...
  const handleDeleteNote = (idx: number) => {
    if (!docRef || !community.journal) return;
    if (window.confirm("Confirm deletion of this note?")) {
      const updatedJournal = community.journal.filter((_: any, i: number) => i !== idx);
      updateDocumentNonBlocking(docRef, { journal: updatedJournal });
