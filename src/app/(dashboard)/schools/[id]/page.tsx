...
export default function SchoolDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const historyItems = useMemo(() => {
    ...
    const journalItems = (school.journal || []).map((j: any, idx: number) => ({
  ...
  const handleAddNote = () => {
    ...
    let updatedJournal = [...(school.journal || [])];
  ...
  const handleDeleteNote = (idx: number) => {
    if (!docRef || !school.journal) return;
    if (window.confirm("Confirm deletion of this note?")) {
      const updatedJournal = school.journal.filter((_: any, i: number) => i !== idx);
      updateDocumentNonBlocking(docRef, { journal: updatedJournal });
