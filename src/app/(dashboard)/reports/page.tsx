    ...
    <div className="max-w-[1440px] mx-auto flex flex-col gap-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="bg-primary/10 border-primary/20 text-primary font-black uppercase text-[8px] tracking-[0.2em] px-2 py-0.5">
              Ops Unit
            </Badge>
            <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Global Analytics</span>
          </div>
          <h1 className="text-3xl font-black tracking-tighter">Performance Intel</h1>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        ...
        <div className="xl:col-span-8 premium-panel rounded-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <h3 className="font-bold text-sm uppercase tracking-widest text-white/90">Expansion Velocity</h3>
          ...
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={growthTrend}>
                ...
                <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 9, fontWeight: 700 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 9, fontWeight: 700 }} />
                <Tooltip contentStyle={{ backgroundColor: 'black', border: '1px solid hsl(var(--border))', borderRadius: '12px', fontSize: '9px' }} />
                <Area type="monotone" dataKey="apps" stroke="hsl(var(--primary))" strokeWidth={3} fillOpacity={1} fill="url(#colorApps)" />
                <Area type="monotone" dataKey="live" stroke="hsl(var(--accent))" strokeWidth={3} fill="transparent" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="xl:col-span-4 premium-panel rounded-2xl p-6 flex flex-col gap-6">
          <div className="flex flex-col">
            <h3 className="font-bold text-sm uppercase tracking-widest text-white/90">Regional Reach</h3>
          ...
          <div className="h-[180px] w-full flex items-center justify-center relative">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={marketShare} cx="50%" cy="50%" innerRadius={55} outerRadius={75} paddingAngle={6} dataKey="value">
            ...
            <div className="absolute flex flex-col items-center justify-center">
              ...
              <span className="text-[7px] font-black uppercase tracking-widest text-muted-foreground">Zones</span>
            </div>
          </div>
            ...
            {marketShare.map((item) => (
              <div key={item.name} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white/80">{item.name} Market</span>
                </div>
        ...
        <div className="xl:col-span-6 premium-panel rounded-2xl p-6 flex flex-col gap-6">
          <h3 className="font-bold text-sm uppercase tracking-widest text-white/90">Funnel Integrity</h3>
          <div className="h-[200px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={funnelData} layout="vertical" margin={{ left: 10 }}>
                <XAxis type="number" hide />
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 9, fontWeight: 800 }} />
                <Bar dataKey="value" fill="hsl(var(--primary))" radius={[0, 6, 6, 0]} barSize={18} opacity={0.8} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {/* Intelligence Card */}
        <div className="xl:col-span-6 premium-panel rounded-2xl p-6 flex flex-col justify-between bg-gradient-to-br from-primary/10 via-background to-accent/5">
          <div className="flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <div className="size-11 rounded-xl bg-white/[0.03] border border-white/10 flex items-center justify-center">
                <Zap className="size-5 text-accent animate-pulse" />
              </div>
              <Badge className="bg-accent/20 text-accent border-none font-black text-[8px] uppercase tracking-widest px-2 py-0.5">AI INSIGHT</Badge>
            </div>
            
            <div className="flex flex-col gap-1.5">
              <h4 className="text-xl font-black tracking-tight">Expansion Acceleration</h4>
          ...
          <div className="flex items-center gap-3 pt-6 border-t border-white/5">
            <Button className="flex-1 h-9 bg-white text-black hover:bg-white/90 font-black uppercase text-[9px] tracking-widest rounded-xl">
              Optimize Strategy
            </Button>
            <Button variant="ghost" className="size-9 rounded-xl border border-white/10 p-0 flex items-center justify-center">
