        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[9px] font-black uppercase tracking-[0.1em] text-muted-foreground/70">Expansion Ops</span>
              <div className="h-0.5 w-0.5 rounded-full bg-white/20" />
              <span className="text-[9px] font-bold text-accent uppercase tracking-widest">{platforms.length} active leads</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative group w-48 mr-1">
            ...
            <Input 
              ...
              className="pl-8 h-8 bg-white/[0.03] border-white/[0.05] rounded-lg font-medium text-[11px] focus-visible:ring-primary/30" 
            />
          </div>
          <Button variant="outline" className="h-8 font-bold gap-1.5 border-white/10 px-4 rounded-lg text-[10px]">
            <Filter className="size-3.5" /> Filters
          </Button>
          <Button className="bg-primary hover:bg-primary/90 text-white font-bold gap-1.5 h-8 px-4 rounded-lg shadow-lg text-[10px]">
        ...
        <TabsContent value="kanban" className="mt-0 outline-none">
          <div className="flex gap-4 overflow-x-auto pb-6 min-h-[60vh] custom-scrollbar px-1">
            {STAGES.map((stage) => {
              ...
              return (
                <div key={stage} className="flex flex-col gap-3 min-w-[260px] w-[260px]">
                  <div className="flex items-center justify-between px-3 py-1.5 bg-white/[0.02] rounded-lg border border-white/[0.05]">
                    <h3 className="text-[9px] font-black uppercase tracking-[0.15em] text-muted-foreground flex items-center gap-1.5">
                      <span className="size-1 rounded-full bg-primary/40" />
                      {stage}
                    </h3>
                    <Badge variant="outline" className="bg-white/5 border-none text-[9px] font-bold text-muted-foreground/60 px-1.5 py-0">
                      {items.length}
                    </Badge>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    {items.length === 0 ? (
                      ...
                      items.map(p => (
                        <div key={p.id} className="glass-card p-4 rounded-xl flex flex-col gap-3 group cursor-pointer active:scale-[0.98] transition-all">
                          ...
                          <div className="flex flex-wrap gap-1">
                            <Badge variant="outline" className="text-[8px] font-bold bg-white/5 border-none text-muted-foreground/80 py-0 uppercase tracking-tighter">{p.type}</Badge>
                          </div>

                          <div className="flex items-center justify-between mt-0.5 pt-3 border-t border-white/[0.03]">
                            <div className="flex items-center gap-1.5">
                              <div className="size-4 rounded-full bg-primary/10 flex items-center justify-center text-[8px] font-black text-primary">
                                {p.owner.charAt(0)}
                              </div>
                              <span className="text-[9px] font-bold text-muted-foreground/70">{p.owner.split(' ')[0]}</span>
                            </div>
                            <div className={cn(
                    ...
                    <Button variant="ghost" className="h-10 border border-dashed border-white/[0.03] hover:border-primary/20 hover:bg-primary/5 rounded-xl font-black text-[9px] uppercase tracking-widest text-muted-foreground/40 hover:text-primary">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/[0.03] border-b border-white/[0.05]">
                  <th className="p-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Platform</th>
                  <th className="p-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Type</th>
                  <th className="p-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Market</th>
                  <th className="p-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Stage</th>
                  <th className="p-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Priority</th>
                  ...
                  <tr key={p.id} className="border-b border-white/[0.02] hover:bg-white/[0.01] transition-colors group cursor-pointer">
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        <div className="size-8 rounded-lg bg-white/[0.03] border border-white/[0.05] flex items-center justify-center font-bold text-xs text-primary group-hover:border-primary/30">
                          {p.name.charAt(0)}
                        </div>
                        <div className="flex flex-col">
                          <span className="font-bold tracking-tight text-xs text-white group-hover:text-primary">{p.name}</span>
                          <span className="text-[8px] font-bold text-muted-foreground/60">{p.owner}</span>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                      <Badge variant="outline" className="bg-white/5 border-none text-[8px] font-black uppercase tracking-tighter text-muted-foreground/80 px-2">{p.type}</Badge>
                    </td>
                    <td className="p-4 text-[11px] font-bold text-white/80">{p.market}</td>
                    <td className="p-4">
                      <Badge className={cn(
                        "font-black text-[8px] uppercase tracking-widest rounded px-2 py-0.5",
                    ...
                    <td className="p-4">
                      <Badge variant="outline" className={cn(
                        "font-black uppercase text-[8px] tracking-[0.15em] rounded px-1.5 border",
